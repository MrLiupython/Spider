...
class Handler(BaseHandler):
    ...
    def get_query_list(self, num, start=None):  # 航线列表
        # # r = requests.get("http://112.74.47.255:8080/api/sp/airlines?carrier=AK&status=1", timeout=15)
        # r = requests.get('http://112.74.47.255:8080/api/sp/airlines_country?ac_type=2&carrier=AK&status=1', timeout=15)  # 跨境
        # # r = requests.get('http://112.74.47.255:8080/api/sp/airlines_country?ac_type=1&carrier=AK&status=1', timeout=15)  # 大陆
        # # r = requests.get('http://112.74.47.255:8080/api/sp/airlines_country?ac_type=3&carrier=AK&status=1', timeout=15)  #海外
        # r.encoding = 'utf-8'
        # airlines = r.json()['data']
        # airlines = ['KUL-LGK', 'DMK-KUL']
